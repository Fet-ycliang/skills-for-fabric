...
## Example 3 — Route a timeseries read to `eventhouse-cli`
...
### Compose KQL and delegate (pattern; `eventhouse-cli` owns the actual `az rest` call)
...
If this returns a non-empty array, surface it to the user: *"This ontology has an invalid `KustoTable` + `NonTimeSeries` binding — Eventhouse sources are TimeSeries-only. I can switch to the `fabriciq-ontology-cli` authoring mode to fix it."* Do **not** try to route a query through such a binding.
