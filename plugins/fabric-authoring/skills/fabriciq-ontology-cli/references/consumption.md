<!-- Mode reference for the `fabriciq-ontology-cli` skill. Loaded on demand from `skills/fabriciq-ontology-cli/SKILL.md` when the request matches the `consumption` mode. -->
...
> 2. This mode is **read-only**. It never calls `createItem` or `updateDefinition`. For schema changes, switch to the **`fabriciq-ontology-cli` authoring mode**.
...
# fabriciq-ontology-cli consumption mode — Fabric Ontology Consumption via CLI
...
| Ontology Definition Tree                         | [ONTOLOGY-AUTHORING-CORE.md § Definition Tree](authoring/ONTOLOGY-AUTHORING-CORE.md#definition-tree)                      | Authoritative file/folder layout of parts you will decode         |
| EntityType & EntityTypeProperty schema           | [ONTOLOGY-AUTHORING-CORE.md § EntityType file](authoring/ONTOLOGY-AUTHORING-CORE.md#entitytype-file--entitytypesiddefinitionjson) | `valueType` catalog, key / display-name contracts                 |
| DataBinding schema + source-type mapping         | [ONTOLOGY-AUTHORING-CORE.md § DataBinding file](authoring/ONTOLOGY-AUTHORING-CORE.md#databinding-file--entitytypesiddatabindingsguidjson) | `LakehouseTable` vs `KustoTable`; `propertyBindings[]` shape       |
| RelationshipType + Contextualization             | [ONTOLOGY-AUTHORING-CORE.md § RelationshipType file](authoring/ONTOLOGY-AUTHORING-CORE.md#relationshiptype-file--relationshiptypesiddefinitionjson) | Source/target + linking-table contract                            |
...
| Ontology Concepts                                | [Ontology Consumption Concepts](#ontology-consumption-concepts)                                                              | Entity / relationship / binding / grounding context               |
| Tool Stack                                       | [Tool Stack](#tool-stack)                                                                                                    |                                                                   |
| Connection                                       | [Connection](#connection)                                                                                                    | Discover workspace, ontology ID; Get Item Definition              |
| Consumption Scope                                | [Consumption Scope](#consumption-scope)                                                                                      | What this skill does / does not do                                |
| Grounding Context Extraction (deep reference)    | [grounding-extraction.md](consumption/grounding-extraction.md)                                                                | Decode parts → grounding JSON for agents                          |
| Query Routing (deep reference)                   | [routing.md](consumption/routing.md)                                                                                          | Binding kind → per-datasource skill + query shape                 |
| Worked Examples                                  | [examples.md](consumption/examples.md)                                                                                        | End-to-end bash recipes (enumerate → route → query)               |
| Graph Walks (N-hop neighborhood from anchor)     | [graph-walks.md](consumption/graph-walks.md)                                                                                  | Anchor entity + hop budget → composed inline reads, no scripts    |
| Snappy-Response Discipline                       | [Snappy-Response Discipline](#snappy-response-discipline)                                                                    | Inline-first; script only when stateful or re-runnable            |
| Must / Prefer / Avoid / Troubleshooting          | [Must / Prefer / Avoid / Troubleshooting](#must--prefer--avoid--troubleshooting)                                             | LLM decision rules                                                |
| Agentic Workflows                                | [Agentic Workflows](#agentic-workflows)                                                                                      | Ground-then-query loop, schema-aware query generation             |
| Agent Integration Notes                          | [Agent Integration Notes](#agent-integration-notes)                                                                          | How this skill composes with authoring / per-datasource skills    |
...
**Grounding context** = the flattened, agent-ready projection of that tree: a JSON summary an LLM can read to decide *which entity type to query*, *which column holds the key*, *which table to hit*, and *which consumption skill to invoke*. Full shape + extraction recipe: [grounding-extraction.md](consumption/grounding-extraction.md).

Property `valueType` allowed values (exact): `String`, `Boolean`, `DateTime`, `Object`, `BigInt`, `Double`. Integers are `BigInt` (not `Int64`); GUIDs are modelled as `String`. See [ONTOLOGY-AUTHORING-CORE.md § EntityTypeProperty](authoring/ONTOLOGY-AUTHORING-CORE.md#entitytypeproperty) for the full source-column → `valueType` mapping.
...
The full fetch-and-decode flow for this mode (LRO capture + part decode + tree reconstruction, Bash + PowerShell + Python helpers) lives in [grounding-extraction.md § Fetch and Decode](consumption/grounding-extraction.md#fetch-and-decode-an-ontology-definition). The `fabriciq-ontology-cli` authoring mode documents the same redirect-host workaround in its [LRO Header Capture section](authoring.md#lro-header-capture-with-az-rest) — keep the two in sync if you change one.
...
Deep recipe + full JSON shape: [grounding-extraction.md](consumption/grounding-extraction.md). Quick index:

| Topic | Reference |
|---|---|
| Fetch + LRO + decode parts | [grounding-extraction.md § Fetch and Decode](consumption/grounding-extraction.md#fetch-and-decode-an-ontology-definition) |
| Reconstruct the definition tree in memory | [grounding-extraction.md § Tree Reconstruction](consumption/grounding-extraction.md#tree-reconstruction) |
| Produce a grounding JSON summary (entities + bindings + relationships) | [grounding-extraction.md § Grounding Summary Schema](consumption/grounding-extraction.md#grounding-summary-schema) |
| Diff two ontology versions | [grounding-extraction.md § Diff Two Ontologies](consumption/grounding-extraction.md#diff-two-ontologies) |

**Grounding JSON contract** — the authoritative shape lives in [grounding-extraction.md § Grounding Summary Schema](consumption/grounding-extraction.md#grounding-summary-schema). Routing decisions read these fields per binding:
...
Deep recipe + per-skill invocation templates: [routing.md](consumption/routing.md). Quick decision table:
...
- **Inline `az rest` / `curl` over Python scripts** for read-only consumption work — graph walks, single-entity lookups, and ad-hoc fan-out reads should compose ≤ ~15 REST calls in the shell. Reach for a script only when the work is **stateful** (envelope assembly, ID maps, LROs that the user wants re-runnable). See [Snappy-Response Discipline](#snappy-response-discipline) and [graph-walks.md](consumption/graph-walks.md).
...
- **Mutating the ontology** from this mode — switch to the `fabriciq-ontology-cli` authoring mode.
...
| Graph walk, hop ≤ 2 (see [graph-walks.md](consumption/graph-walks.md)) | ✅ | ❌ |
...
| Ontology authoring / mutation (35-part envelope, ID cross-refs) | ❌ | ✅ (switch to `fabriciq-ontology-cli` authoring mode) |
...
Generated KQL (delegated to eventhouse-cli):
  AircraftReadings
  | where AssetId == "N42ZA"
...
Full end-to-end bash recipes (enumerate → ground → route → query) live in [examples.md](consumption/examples.md). For "show me everything related to X" prompts, jump straight to the N-hop walk in [graph-walks.md](consumption/graph-walks.md).
...
For hop=1 this is typically ≤ 10 round trips and stays inline. Full algorithm, fan-out template, hop budget, and a worked Panel7 example live in [graph-walks.md](consumption/graph-walks.md).

---

## Examples

End-to-end worked examples (enumerate an ontology → build grounding JSON → route a source-table query to the correct sibling consumption skill → traverse a relationship across Lakehouse + Eventhouse) live in [examples.md](consumption/examples.md). N-hop neighborhood walks from an anchor instance (Panel7-style) live in [graph-walks.md](consumption/graph-walks.md). Complete fetch-and-decode scripts live in [grounding-extraction.md](consumption/grounding-extraction.md). Per-binding-type invocation templates live in [routing.md](consumption/routing.md).
