...
> **Speed-first:** every step in this file is a single REST call (one `az rest` or one `curl`). Do **not** distill into a Python script unless the user asks for a reusable artifact. See [consumption mode § Snappy-Response Discipline](../consumption.md#snappy-response-discipline).
...
3. **Project bound columns only.** Never `SELECT *` — the binding is the column whitelist (see [consumption mode § Prefer](../consumption.md#prefer)).
