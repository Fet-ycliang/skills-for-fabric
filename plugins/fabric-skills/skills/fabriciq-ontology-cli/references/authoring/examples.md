...
```bash
# Prereqs: WS_ID, LH_ID resolved (see authoring.md § Connection)
...
EOF
# To create inside a workspace folder, resolve FOLDER_ID per authoring.md § Connection → Folder
...
> Assumes Example 2 has already been applied (so `Aircraft` has a `NonTimeSeries` binding — required before any timeseries binding). `EH_ID`, `CLUSTER_URI`, `DB_NAME` are resolved per the authoring.md § Connection → Eventhouse recipe.

```bash
# Prereqs: WS_ID, ONTO_ID, AIRCRAFT_ET_ID, AIRCRAFT_KEY_PROP_ID from Example 2;
#          EH_ID, CLUSTER_URI, DB_NAME from authoring.md § Connection → Eventhouse.
